...
func _apply_damage_to_target(target:Node3D) ->void:
	var parrent_target = target.get_owner()
	var health_component = parrent_target.get_node_or_null("HealthComponent")
	if health_component and health_component.has_method("take_damage"):
		health_component.take_damage(gun_recource.damage,parrent_target)
